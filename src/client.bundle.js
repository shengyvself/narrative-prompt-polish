...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── 多语言（zh/en 按 document.lang，Max-Null 同款）────────────
    var STRINGS = {
      zh: {
    ...
    function buildPolishTaskbook(draft) {
      ...
      return [
        ...
        draft || "（空）",
        "",
        "请按 polish 设计意图（4 类意图骨架 debug/implement/explain/chat + LCQ 共享重写规则）将上面草稿改写为可直接交给 AI 助手执行的高质量提示词。要求：保留事实、模糊→具体动作、缺背景标注待确认、拆编号子问题、长度匹配复杂度。完成后输出最终润色文本（不要解释过程）。"
      ].join("\n");
    }
