...
/**
 * 挂载前置服务。sessions = C 方案 live 会话读取（deriveMessages/requestHeader）；
 * agentDefaultModel = 渠道最终兜底；agents = 主会话 live Agent（主路径子代理的 parent）；
 * subagents = continuable 子代理编排（polish.start）；cordis fiber 守卫要求显式声明。
 */
export const inject = ['webServer', 'sessionQuery', 'sessions', 'llm', 'loader', 'agentDefaultModel', 'agents', 'subagents']
...
/**
 * settings 注册 schema（schemastery 构造；全部带 DEFAULTS 缺省值）。
 * 调用方已保证 z 可用。
 */
